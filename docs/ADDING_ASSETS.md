...
10 種だけでは寂しいので、**全 100 種ぶんの写真を一括でローカルに落とす**スクリプトを用意しています。
有名な恐竜は Wikidata に画像があるため、実行するとカードの大半が写真で埋まります。

```bash
# Wikimedia に到達できるネットワーク（自宅 / 会場など）で実行
npm run fetch:images

# オプション
node scripts/fetch-images.mjs --limit 10   # 先頭10種だけ試す
node scripts/fetch-images.mjs --force      # 取得済みも取り直す
```

- `frontend/public/assets/images/` に画像を保存し、`manifest.json` に**著作者・ライセンスつきで**自動登録します
- 取得できなかった種はプレースホルダー（分類別シルエット）のまま
- **文化祭当日はオフライン運用が安全**なので、事前にこのスクリプトで画像を落として
  コミットしておくのがおすすめです（当日の会場 Wi-Fi に依存しなくなります）
- 社内プロキシ等で Wikimedia がブロックされている環境では実行できません。到達できる回線で実行してください

> 個別に良い写真へ差し替えたいときは、下記「ローカル差し替え」で `manifest.json` を手で上書きできます。
